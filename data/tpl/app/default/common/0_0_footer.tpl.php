<?php defined('IN_IA') or exit('Access Denied');?>		<?php  if(empty($footer_off)) { ?>
  ...
  'acid' => 0,
	...
	$(function(){
		...
		<?php  } ?>
		<?php  if(!empty($_GPC['eid']) || !empty($_GPC['m'])) { ?>
			var visit_url = "<?php  echo url('utility/visit/update', array('m' => $_W['current_module']['name']));?>";
		<?php  } else { ?>
			var visit_url = "<?php  echo url('utility/visit/update', array('m' => 'wesite'));?>";
		<?php  } ?>
		$.post(visit_url, function(data){});
